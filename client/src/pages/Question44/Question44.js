...
function Question44 (){
  ...
  const [second44Status, setSecond44Status] = useState("");
  ...
  const second43 = (e) => {
    e.preventDefault();
    Axios.post("http://localhost:3001/second43", {
      heading: heading,
      ...
      if(response.data.message){
        setSecond44Status(response.data.message);
      }else{
        setSecond44Status("QUIZ QUESTION AND ANSWER CREATED SUCCESSFULLY");
...
<br/>
   ...
   <div className="loginForm">
        <form>
          ...
          <h4>Question 4</h4>
          <br/>
          <div  className="cnav"><a href="/question54" className="btn btn-secondary ">New Question</a>
...
<ReactQuill theme="snow"  value={answer1} onChange= {setAnswer1} placeholder="Enter Answer"  />
    ...
    <ReactQuill theme="snow" value={map1} onChange={setMap1} placeholder="Map Outcome" />         
          ...
          <br/>
          <input className=".btn-btn btn-primary" type="submit" onClick={second43} value="Create Question page" />
          <h1 style={{fontSize: '15px', textAlign: 'center', marginTop:'20px'}}>{second44Status}</h1>
        </form>
        </div>
