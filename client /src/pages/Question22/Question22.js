...
function Question22 (){
  ...
  const [second21Status, setSecond21Status] = useState("");
  ...
  const second21 = (e) => {
    e.preventDefault();
    Axios.post("http://localhost:3001/second21", {
      heading: heading,
      ...
      if(response.data.message){
        setSecond21Status(response.data.message);
      }else{
        setSecond21Status("QUIZ QUESTION AND ANSWER CREATED SUCCESSFULLY");
      }
    })
  }
...
<div>
                ...
                <button  className="button-urls"onClick={setUrl} >Publish Url</button>
                    {
                      url &&(<p>http://localhost:3000/page21211</p>)
                    }
                      <br/>
                      <br/>  
                </div>
...
<br/>
   ...
   <div className="loginForm">
        <form>
          ...
          <h4>Question 2</h4>
          <br/>
          <div  className="cnav"><a href="/question32" className="btn btn-secondary ">New Question</a>
...
<option value="/question22">2</option>

<option value="/question24">4</option>
</select>
  ...
  <br/>
   ...
   <label htmlFor="map">Outcome1*</label>
    ...
    <br/>
     ...
     <br/>
         <input className=".btn-btn btn-primary" type="submit" onClick={second21} value="Create Question page" />
          <h1 style={{fontSize: '15px', textAlign: 'center', marginTop:'20px'}}>{second21Status}</h1>
        </form>
        </div>
